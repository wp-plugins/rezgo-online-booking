...
<link media="all" href="http://<?=$_SERVER[HTTP_HOST]?><?=$site->path?>/header.css" type="text/css" rel="stylesheet">
<link media="all" href="http://<?=$_SERVER[HTTP_HOST]?><?=$site->path?>/pdf.css" type="text/css" rel="stylesheet">

<div id="rezgo" class="wrp_pdf">	
	
<div id="panel_full">

	<? 
		$order = $site->getBookings('t=order_code&q='.$trans_num);
		
		$count = count($order);
		
		if(!$order) { $site->sendTo("/tour"); }
	...
	<!-- use div class="btn_print_one" when you only show invoce btn as below -->
	 <!--<div class="btn_print_one"><h1 class="print_invoice"><a href="#">Click to Print a Invoice</a></h1><h1 class="print_voucher"><a href="#">Click to Print a Voucher</a></h1></div>-->
	  ...
	  <div class="tour_info">
	  <h2>Booking <?=$c?> of <?=$count?> (booked on <?=date("F d, Y", (int)$booking->date_purchased)?> / local time)</h2>
	  <ol class="tour_receipt">
	    ...
	    <li class="info"><label>Itinerary</label><span><?=$item->schedule->itinerary?></span></li>
...
<div id="rezgo_logo"><a href="http://www.rezgo.com" target="_blank" title="powered by rezgo">powered by<img src="http://<?=$_SERVER[HTTP_HOST]?><?=$site->path?>/images/logo_rezgo.gif" border="0" alt="Rezgo" /></a></div>
<!-- Rezgo logo DO NOT DELETE -->
</div><!--end rezgo wrp-->
